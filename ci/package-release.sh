...
#     Keeping the names stable lets the whole bundle ship intact.
#   * the mcode wrapper (interactive_pkg_mcode.vhdl) names the library in its
...
#     -sv_lib / --load / -m / -L reference; versions coexist as sibling folders.
#
...
STAGE="$OUT/interactive-sim-$VER-$PLAT"
rm -rf "$STAGE"
mkdir -p "$STAGE"
...
tar -C "$OUT" -czf "$OUT/interactive-sim-$VER-$PLAT.tar.gz" "interactive-sim-$VER-$PLAT"
echo "packaged $OUT/interactive-sim-$VER-$PLAT.tar.gz"
ls -l "$STAGE"
