#!/usr/bin/env python3
"""ulx3s_viewer.py -- a config-driven graphical board viewer for interactive-sim.

It draws nothing itself: a JSON config supplies a board *photo* (by URL or local
path) and a pixel-coordinate map of where the LEDs and buttons are on that photo.
The viewer then speaks the same wire protocol as the reference terminal viewer
(viewer/interactive_viewer.py): it LISTENS on a TCP port, the simulation connects
to it (set INTERACTIVE_STREAM=host:port for the sim), and then:

  * design-driven `flag` events light an LED overlay at its photo position, in the
    configured "on" colour;
  * a mouse click is hit-tested against the button regions in the map; the button
    under the cursor sends `val=1` on press and `val=0` on release, so it behaves
    as a momentary push-button (a `"toggle": true` button flips and latches).

Config (see viewer/ulx3s.json) -- all coordinates are in ORIGINAL image pixels;
`image.scale` scales the photo and every coordinate together for display:

  {
    "image":  { "url": "...jpg" | "/path", "cache": "name.jpg", "scale": 0.62 },
    "leds":   { "on_color": "#ff5a36", "radius": 13, "glow": true,
                "items": [ { "name": "leds", "bit": 0, "x": 632, "y": 422 }, ... ] },
    "buttons":[ { "name": "btn_pwr", "shape": "circle", "x": 1400, "y": 343,
                  "r": 42, "key": "p" },
                { "name": "sw0", "shape": "rect", "x": 800, "y": 250,
                  "w": 30, "h": 30, "toggle": true } ]
  }

An LED item lights when bit `bit` of flag `name` is 1, so an N-bit bus ("leds")
is N items with different bit indices, and a 1-bit flag ("led3") is one item.

Building a map for a new photo: run with --calibrate and click on the photo; each
click prints the original-image pixel coordinate to the console (and marks it),
so you can read off the x/y for every LED and button.

JPEG photos need Pillow (`pip install pillow`); PNG/GIF also work with stdlib Tk.

Usage:
  python ulx3s_viewer.py [--config viewer/ulx3s.json] [--port 7777] [--host 0.0.0.0]
                         [--refresh] [--calibrate]
"""
...
# -- palette for the non-photo chrome (status strip, calibration marks) --------
BG          = "#101418"
PANEL_BG    = "#161b20"
TEXT        = "#cfd6df"
TEXT_DIM    = "#7e8893"
...
def load_photo(path, scale, crop=None):
    ...
    try:
        ...
        if crop:
            raise SystemExit("[viewer] image.crop needs Pillow "
                             "(pip install pillow)")
        if scale < 1.0:
...
# ---------------------------------------------------------------------------
# Board: places LED/button overlays from the config over the photo
# ---------------------------------------------------------------------------
class Board:
    ...
    def _build_leds(self, leds):
        ...
        for item in leds.get("items", []):
            ...
            gid = -1
            if glow:
                gid = mk(*gbox, fill=color, outline="", stipple="gray50",
                         state="hidden")
            cid = mk(*box, fill=color, outline=LED_OFF_RIM, width=1,
                     state="hidden")
            self.leds.setdefault(item["name"], []).append(
                (item.get("bit", 0), cid, gid))
        ...
        for b in buttons:
            ...
            if shape == "rect":
                ...
                ov = self.c.create_rectangle(x - hw, y - hh, x + hw, y + hh,
                                             outline=CAL_COLOR, width=1,
                                             fill=CAL_COLOR, stipple="gray25",
                                             state="hidden")
            else:
    ...
    def _activate(self, b):
        if b["toggle"]:
            b["state"] ^= 1
            self.c.itemconfig(b["overlay"],
                              state="normal" if b["state"] else "hidden")
...
class App:
    def __init__(self, cfg_path, host, port, refresh=False, calibrate=False):
        ...
        if cache:                                  # cache next to the config file
            cache = os.path.join(os.path.dirname(os.path.abspath(cfg_path)), cache)

        self.root = tk.Tk()
        self.root.title("interactive-sim · ULX3S viewer"
                        + ("  [CALIBRATE]" if calibrate else ""))
        self.root.configure(bg=BG)

        path = fetch_bytes(img["url"], cache, refresh)
        self.photo, w, h = load_photo(path, scale, crop)
        ...
        self.board = Board(self.canvas, cfg, scale, self.send_control,
                           self.crop_origin)
        self.scale = scale
        ...
        if calibrate:
            ...
            self.canvas.bind("<ButtonPress-1>",
                             lambda e: self.board.press_xy(e.x, e.y))
            self.canvas.bind("<ButtonRelease-1>",
                             lambda _e: self.board.release())
    ...
    def send_control(self, name, val):
        ...
        if conn is None:
            return
        try:
            conn.sendall((json.dumps({"name": name, "val": int(val)}) +
                          "\n").encode("utf-8"))
        except OSError:
            pass
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Config-driven ULX3S board viewer "
                                             "for interactive-sim.")
    ap.add_argument("--config", default=os.path.join(here, "ulx3s.json"),
                    help="board config JSON (default: viewer/ulx3s.json)")
    ...
    args = ap.parse_args()
    App(args.config, args.host, args.port, args.refresh, args.calibrate).run()
